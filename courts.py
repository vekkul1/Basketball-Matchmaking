...
# used to import 12 courts from around the helsinki area
def init():
    import sqlite3
    dbb = sqlite3.connect("database.db")

    sql = """SELECT count(*) FROM locations"""
    if dbb.execute(sql).fetchone()[0] == 0: # runs if locations table is empty
        print("initializing locations table")
